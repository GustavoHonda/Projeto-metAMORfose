...
def generate_mock_respostas(num_rows = 50):
    ...
    for i in range(1, num_rows + 1):
        ...
        atendimento = random.choice(["Terapia online", "Terapia presencial"])
        preco = random.choice([30, 50, 60, 70, 80, 90, 100])
        urgencia = random.choice(["Imediato", "Essa semana", "O quanto antes"])
        genero = random.choice(genders)
        
        row = [
            i,
            fake.date_time_this_year().strftime(f"%Y-%m-%d %H:%M:%S"),
            nome,
            email,
            f"wa.me/55{telefone}",
            area,
...
def generate_mock_professionals(n=50, seed=42):
    ...
    areas = [
        "Psychotherapy", "Nutrition", "General Practice", "Personal Training",
        "Holistic Therapy", "Occupational Therapy", "Speech Therapy", "Acupuncture"
    ]
    ...
    for i in range(1, n + 1):
        ...
        price = random.choice([30, 50, 60, 70, 80, 90, 100])
